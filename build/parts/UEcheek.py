...
class UEcheek(UEface):
    ...
    def build(self):
        ...
        side = prefix.split('_')[-1]  # "L"
        grp = mc.group(empty=True, name=f'{prefix}_extra_offsets' )
        if self.NL == False:
            top_group = None
        for item in [top_group, f'{prefix}_NLFold_ribbon', f'{prefix}_NLFold_01_{side}_CTRL_CNST_GRP', f'{prefix}_NLFold_02_{side}_CTRL_CNST_GRP', f'{prefix}_NLFold_03_{side}_CTRL_CNST_GRP', f'{prefix}_NLFold_04_{side}_CTRL_CNST_GRP', f'{prefix}_NLFold_05_{side}_CTRL_CNST_GRP', f'{prefix}_NLFold_02_Major_jnt', f'{prefix}_NLFold_04_Major_jnt', f'{prefix}_NLFold_05_Major_jnt',]:
            try:
                mc.parent(item, grp)
            except:
                pass
